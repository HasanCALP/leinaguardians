...
exports.run = async(client, message, args) => {
	...
	if(!banlog) return message.channel.send('Ban log kanalı ayarlanmamış.')
    let user = message.mentions.users.first()
    let sebep = args.slice(1).join(' ') || "Belirtilmemiş."
     if(!user) return message.channel.send(':x: Bir kişi etiketlemelisin.')
     if(user.id === message.author.id) return message.channel.send(':x: Kendini banlayamazsın.')
     if(user.id === client.user.id) return message.channel.send(':x: Botu banlayamazsın.')
  if(user.id === message.guild.ownerID) return message.channel.send(':x: Sunucu sahibini banlayamazsın.')
   ...
   message.channel.send('<@'+ user.id + '> Kullanıcısını **'+ sebep+ '** Sebebiyle engellemek istediğine eminmisin ?').then(async m => {
   	 m.react('✅').then(r =>{ 
  ...
  message.guild.members.cache.get(user.id).ban({
   ...
   tamam2.on('collect', async (r)=>{
     m.delete()
message.channel.send('Engelleme işlemi iptal edildi.')
      })
    })
 })
} 
